...
echo -e "${YELLOW}[1/4] Checking for .env file...${NC}"
if [ ! -f .env ]; then
    cp .env.example .env
...
echo -e "${YELLOW}[2/4] Starting Docker services (PostgreSQL, Redis, Backend)...${NC}"
docker-compose -f docker-compose.local.yml up --build -d
echo -e "${GREEN}      Docker services started in background.${NC}"
echo ""

# Step 3: Start frontend server
echo -e "${YELLOW}[3/4] Starting frontend server on port 3000...${NC}"
